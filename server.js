...
// === ZMODYFIKOWANA FUNKCJA GENEROWANIA INSEKTÓW ===
function generateInsects(roomId, groundLevel, worldWidth) {
    ...
    // ZMIANA: Ustalenie nowych, bardziej restrykcyjnych granic respu motyli
    // Górna granica respu (Y rośnie w dół, więc to jest "najwyżej" na ekranie)
    const minY = DEDICATED_GAME_HEIGHT * 0.1;
    // Dolna granica respu (tuż nad ziemią)
    const maxY = DEDICATED_GAME_HEIGHT *0.7;

    for (let i = 0; i < numInsects; i++) {
        const startX = seededRandom() * worldWidth;
        // ZMIANA: Losowanie pozycji startowej Y w nowym, ograniczonym zakresie
        ...
        insects.push({
            ...
            x: startX,
            y: startY,
            hue: seededRandom() * 360,
            angle: 0,
            animationFrame: 0,
            // Parametry dla ruchu sinusoidalnego - klient ich użyje do obliczeń
            timeOffset: seededRandom() * 1000,
            anchorX: startX,
            baseY: startY, // Zapisz nową pozycję startową jako bazową
            drift: (seededRandom() - 0.5) * 2.5,
            hSpeed: 0.6 + seededRandom() * 0.3,
            vSpeed: 0.5 + seededRandom() * 0.2,
            hAmp: 50 + seededRandom() * 130,
            vAmp: 30 + seededRandom() * 140,
        });
    }
    return insects;
}
        ...
        for (const playerId in room.players) {
            ...
            if (room.gameData.groundPlants) {
                room.gameData.groundPlants.forEach(grass => {
                    ...
                    const isPlayerWalking = Math.abs(player.velocityX) > MIN_VELOCITY_FOR_WALK_ANIMATION;
                    if (!grass.swaying && isPlayerWalking) {
                        const grassHitbox = {
                            x: grass.x,
                            y: grass.y - 20,
