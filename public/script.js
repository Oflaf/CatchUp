...
class CloudManager {
    ...
    draw(ctx, nightAlpha = 0) {
        ...
        const nightTopColor = '#0d1b2a';    
        const nightBottomColor = '#1b263b'; 
...
class WalkingParticleManager {
    ...
    spawn(x, y, isSmall = false) {
        ...
        this.particles.push({
            type: 'walk', // Dodajemy typ, aby odróżnić cząsteczki
            img: image,
    ...
    spawnCampfireSmoke(x, y) {
        const image = Math.random() < 0.5 ? this.particleImages['16x16'] : this.particleImages['8x8'];
        if (!image || !image.complete) return;

        const life = 3.5 + Math.random() * 1.5;
        this.particles.push({
            type: 'campfire', // Typ: dym z ogniska
            img: image,
            initialX: x + (Math.random() - 0.5) * 20,
            y: y,
            alpha: 0.1 + Math.random() * 0.2, // Startuje półprzezroczysty
            scale: 4 + Math.random() * 2, // Startuje duży
            rotation: Math.random() * Math.PI * 2,
            life: life,
            initialLife: life, // Zapamiętujemy początkowy czas życia
            
            rotationSpeed: (Math.random() - 0.5) * 1.5,
            growthSpeed: -(Math.random() * 0.3 + 0.5), // Ujemna wartość = kurczenie się
            fadeSpeed: 1.0, // Stała prędkość zanikania
            velocityY: -(Math.random() * 30 + 20), // Wolniejsze unoszenie
            
            // Właściwości dla ruchu sinusoidalnego
            waveFrequency: Math.random() * 3 + 2,
            waveAmplitude: Math.random() * 12 + 8,
            
            layer: 'behind' // Dym z ogniska zawsze jest z tyłu
        });
    }
    ...
    update(deltaTime) {
        for (let i = this.particles.length - 1; i >= 0; i--) {
            ...
            // <-- NOWA ZMIANA: Logika ruchu sinusoidalnego -->
            if (p.type === 'campfire') {
                const elapsedTime = p.initialLife - p.life;
                p.x = p.initialX + Math.sin(elapsedTime * p.waveFrequency) * p.waveAmplitude;
            }
...
const walkingparticles = new WalkingParticleManager(); // <-- NOWA ZMIANA
...
function loadImages(callback) {
    ...
    const basePaths = { ...customizationUIPaths, ...tutorialImagePaths, ...fishingUIPaths, ...baitImagePaths, ...hookImagePaths, ...weatherImagePaths, ...mineralImagePaths, ...dynamicObjectImagePaths, ...fishingTutorialIconPath, ...fishingTutorialImagePaths, ...walkingParticleImagePaths }; // <-- NOWA ZMIANA
    ...
    for (const h in allPaths) {
        ...
        if (h === 'notice_board') {
            ...
            f();
                    if (walkingParticleImagePaths[h]) { // <-- NOWA ZMIANA
                walkingparticles.particleImages[h] = i; // <-- NOWA ZMIANA
            } // <-- NOWA ZMIANA
...
function gameLoop(currentTime) {
    ...
    // Logika generowania cząsteczek
    if (currentRoom && currentRoom.gameData) {
        const groundLevel = DEDICATED_GAME_HEIGHT - currentRoom.gameData.groundLevel;
        const isPlayerCurrentlyOnGround = localPlayer.y + playerSize >= groundLevel - 1;

        // Cząsteczki przy lądowaniu
        if (isPlayerCurrentlyOnGround && !wasPlayerOnGround) {
            const playerFeetX = localPlayer.x + playerSize / 2;
            const playerFeetY = localPlayer.y + playerSize;
            for (let i = 0; i < 8; i++) { // Stwórz serię dymków przy lądowaniu
                walkingparticles.spawn(playerFeetX, playerFeetY, Math.random() > 0.5);
            }
        }

        // Cząsteczki przy chodzeniu
        if (localPlayer.isWalking && isPlayerCurrentlyOnGround) {
            walkParticleTimer -= deltaTime;
            if (walkParticleTimer <= 0) {
                walkParticleTimer = 0.03; // Twórz dymek co 100ms
                const playerFeetX = localPlayer.x + playerSize / 2;
                const playerFeetY = localPlayer.y + playerSize;
    ...
    cycleManager.update(deltaTime);
        ...
        walkingparticles.update(deltaTime); // <-- NOWA ZMIANA: Aktualizuj dymki
        birdManager.update(deltaTime, cameraX, canvas.width);
...
walkingparticles.draw(ctx, 'behind'); 
    ...
    if (!screenshotMode) {
        // <-- ZMIANA 2: Rysuj przednie dymki PO graczu -->
        walkingparticles.draw(ctx, 'front');
...
joinGameBtn.addEventListener('click', () => {
    ...
    document.querySelector('.server-stats').classList.add('hidden'); // <-- DODANA LINIA
...
function initializeSignaling() {
    ...
    const unavailableRoomCountDisplay = document.getElementById('unavailable-room-count-display');
    ...
    // <-- POCZĄTEK NOWEGO FRAGMENTU -->
    // Nasłuchuj na unikalny numer gościa
    signalingSocket.on('visitorNumber', (number) => {
        const visitorDisplay = document.getElementById('visitor-counter-display');
        if (visitorDisplay) {
            visitorDisplay.textContent = `Hello! You are guest #${number}!`;
        }
    });
    // <-- KONIEC NOWEGO FRAGMENTU -->

    signalingSocket.on('roomListUpdate', (data) => {
        const hosts = data.hosts || {};
        const totalPlayers = data.totalPlayers || 0;

        availableRooms = hosts;
        console.log('Updated room list received:', hosts);

        const allRooms = Object.values(hosts);
        const joinableRoomsCount = allRooms.filter(room => room.playerCount < MAX_PLAYERS_PER_ROOM).length;
        const fullRoomsCount = allRooms.length - joinableRoomsCount;

        if (roomCountDisplay) {
            roomCountDisplay.textContent = `Online chat-rooms: ${joinableRoomsCount}`;
        }
        if (unavailableRoomCountDisplay) {
            unavailableRoomCountDisplay.textContent = `Unavailable rooms: ${1+fullRoomsCount}`;
        }
        if (playerCountDisplay) {
            playerCountDisplay.textContent = `Online players: ${totalPlayers+1}`;
        }
    });
...
function leaveCurrentRoomUI() {
    ...
    document.querySelector('.server-stats').classList.remove('hidden'); // <-- DODANA LINIA
