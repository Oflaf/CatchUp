...
const PLAYER_WALK_SPEED = 14; // Zsynchronizowane z serwerem dla logiki animacji
const MIN_VELOCITY_FOR_WALK_ANIMATION = 0.5; // Zsynchronizowane z serwerem dla logiki animacji
...
let playersInRoom = {};
let insectsInRoom = []; // ZMIANA: Ta tablica jest teraz aktualizowana tylko przy dołączeniu do pokoju.
...
function drawPlayer(p) {
    ...
    // ZMIANA: Stan animacji jest teraz obliczany i przechowywany lokalnie w obiekcie gracza 'p'
...
// --- Główna pętla gry klienta (teraz także do obliczeń animacji) ---
function gameLoop(currentTime) {
    ...
    // === NOWA SEKCJA: Aktualizacje animacji po stronie klienta ===

    // 1. Aktualizacja motyli (ruch, rotacja, klatka animacji)
    if (insectsInRoom.length > 0 && currentRoom.gameData) {
        const worldWidth = currentRoom.gameData.worldWidth || 0;
        const groundLevel = currentRoom.gameData.groundLevel || 0;
        const INSECT_TILE_SIZE = 32;
        const INSECT_MAX_ROTATION_DEGREES = 50;
        const INSECT_ANIMATION_SPEED_TICKS = 8;
        const INSECT_TOP_BOUNDARY_FACTOR = 0.8;

        const insectMinY = DEDICATED_GAME_HEIGHT * (1.0 - INSECT_TOP_BOUNDARY_FACTOR);
        ...
        insectsInRoom.forEach(insect => {
            ...
            const offsetX = Math.sin(time * (insect.hSpeed || 1)) * (insect.hAmp || 100);
            const offsetY = Math.cos(time * (insect.vSpeed || 1)) * (insect.vAmp || 80);

            insect.x = (insect.anchorX || 0) + offsetX;
            insect.y = (insect.baseY || 500) + offsetY;

            insect.y = Math.max(insectMinY, Math.min(insectMaxY, insect.y));

            const currentHorizontalVelocity = Math.cos(time * (insect.hSpeed || 1)) * (insect.hAmp || 100) * (insect.hSpeed || 1);
            insect.angle = currentHorizontalVelocity * 0.5;
            ...
            insect.animationFrame = (insect.animationFrame + 1) % (INSECT_ANIMATION_SPEED_TICKS * 2);
        });
    }

    // 2. Aktualizacja animacji wszystkich graczy (na podstawie danych z serwera)
    if (Object.keys(playersInRoom).length > 0 && currentRoom.gameData) {
        const groundLevel = currentRoom.gameData.groundLevel || 0;
        const groundY_target_for_player_top = DEDICATED_GAME_HEIGHT - groundLevel - playerSize;
...
// ZMIANA: Usunięto listener 'insectsUpdate', ponieważ klient nie otrzymuje już tych danych.
...
socket.on('roomJoined', (roomData) => {
    ...
    if (roomData.gameData) {
        ...
        // ZMIANA: Insekty są teraz odbierane tylko tutaj, na początku gry.
...
socket.on('playerMovedInRoom', (allPlayersData) => {
    if (Array.isArray(allPlayersData)) {
        // ZMIANA: Zamiast po prostu nadpisywać, zachowujemy lokalne klatki animacji
        const updatedPlayersMap = {};
        for (const serverPlayer of allPlayersData) {
            const clientPlayer = playersInRoom[serverPlayer.id];
            if (clientPlayer) {
                // Zachowaj klatki animacji z istniejącego obiektu
                serverPlayer.animationFrame = clientPlayer.animationFrame || 0;
                serverPlayer.idleAnimationFrame = clientPlayer.idleAnimationFrame || 0;
            } else {
                // Nowy gracz, zainicjuj klatki animacji
                serverPlayer.animationFrame = 0;
                serverPlayer.idleAnimationFrame = 0;
            }
            updatedPlayersMap[serverPlayer.id] = serverPlayer;
        }
        ...
        if (localPlayer.id && playersInRoom[localPlayer.id]) {
            ...
            // Zachowaj stan, który jest zarządzany tylko przez klienta
            ...
            // Zaktualizuj obiekt localPlayer, w tym zachowane klatki animacji
            Object.assign(localPlayer, serverState);

            // Przywróć stan zarządzany lokalnie
