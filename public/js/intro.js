document.addEventListener('DOMContentLoaded', () => {
    ...
    const MAX_FINAL_LETTER_HEIGHT = 80;
    ...
    // ZMIANA: Nowa stała do kontrolowania przesunięcia logo w poziomie.
    // Dodatnia wartość przesuwa w prawo, ujemna w lewo.
    // Wartość jest mnożnikiem wysokości litery.
    const LOGO_HORIZONTAL_OFFSET_MULTIPLIER = 0.3; 
    ...
    style.textContent = `
        ...
        .intro-logo {
            transition: transform ${SPREAD_DURATION_MS}ms cubic-bezier(0.68, -0.55, 0.27, 1.55), 
                        opacity ${SPREAD_DURATION_MS * 0.7}ms ease-in-out;
            transform-origin: center;
        }
        @keyframes gentleWobble {
            0% { transform: rotate(0deg); }
            25% { transform: rotate(-${LOGO_WOBBLE_DEG}deg); }
            75% { transform: rotate(${LOGO_WOBBLE_DEG}deg); }
    ...
    function calculateBaseTextMetrics() {
        ...
        totalWidthRatio -= LETTER_SPACING;
    ...
    function recalculateLayout() {
        ...
        animationState.letters.forEach(letterData => {
            ...
            if (letterData.isZ) {
                ...
                // ZMIANA: Obliczamy przesunięcie w pixelach i zapisujemy je do późniejszego wykorzystania
                const logoHorizontalOffset = finalLetterHeight * LOGO_HORIZONTAL_OFFSET_MULTIPLIER;
                animationState.logo.horizontalOffset = logoHorizontalOffset;

                animationState.logo.targetWidth = finalLogoHeight;
                animationState.logo.targetHeight = finalLogoHeight;
                // ZMIANA: Dodajemy obliczone przesunięcie do pozycji `left` logo
                animationState.logo.targetLeft = zCenterX - (finalLogoHeight / 2) + logoHorizontalOffset;
    ...
    function prepareLettersAndLayout() {
        ...
        for (const char of textChars) {
            ...
            if (isZ) {
                ...
                logoWrapper.style.transform = 'scale(0)';
    ...
    function spreadLettersAnimation() {
        ...
        // ZMIANA: Odczytujemy zapisane przesunięcie i obliczamy asymetryczne rozsunięcie
        const offset = animationState.logo.horizontalOffset || 0;
        const shiftLeft = (logoWidth / 2 - offset) - (zWidth / 2);
        const shiftRight = (logoWidth / 2 + offset) - (zWidth / 2);
        ...
        animationState.letters.forEach((letter, index) => {
            ...
            // ZMIANA: Używamy nowych, asymetrycznych wartości przesunięcia
            if (index < zLetterIndex) {
                newTargetX -= shiftLeft + (letter.targetH * 0.1);
            } else {
                newTargetX += shiftRight + (letter.targetH * 0.1);
            }
            letter.targetX = newTargetX;

            letter.element.style.transition = `transform ${SPREAD_DURATION_MS}ms ease-out`;
        ...
        if (animationState.logo) {
            ...
            logo.wrapper.style.transform = 'scale(1)';
            logo.element.style.animation = `gentleWobble 2200ms ease-in-out infinite`;
        }
    ...
    function finalSplitAndZoom() {
        ...
        const moveLetters = (letters, direction) => {
            letters.forEach(letter => {
                ...
                const finalX = direction === 'left' ? -(letter.targetW + 100) : window.innerWidth + 100;
